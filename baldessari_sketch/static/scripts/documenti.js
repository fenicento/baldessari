...
function docGalleria(data){
	...
	var scaleFactor = Math.random() + 1;
	var imgWidth = 100 * scaleFactor;
	var imgHeight = 70 * scaleFactor;
	...
	var img = item.append('a').attr("href", function(d){
					...
					.append('img').attr('src', function(d){
							...
							// }).style("width", imgWidth + "px").style("height", imgHeight + "px")
...
var ddpowerzoomer={
	...
	movemagnifier:function(e, moveBol, zoomdir){
		...
		if (moveBol==true){
			if (e.pageX>=coords.left && e.pageX<=coords.right && e.pageY>=coords.top && e.pageY<=coords.bottom)  //if mouse is currently within boundaries of active base image
